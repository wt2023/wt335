...
bool stopEvent = false;                                     // Setting it to true stops the server   改成true就停止服务

LoginDatabaseWorkerPool LoginDatabase;                      // Accessor to the authserver database   访问/连接auth数据库

/// Handle authserver's termination signals     处理authserver的结束信号
...
/// Print out the usage string for this program on the console.在控制台上打印此程序的用法字符串
...
extern int main(int argc, char** argv)
{
   ...
   // sLog->outString(("║     █╔═╝█╔ ██╗█╔══█╗╚═█╔╝█╔══╝█╔═█║      ║"));
    ...
    //sLog->outString(("║                       ████╗╚███╔╝█║  █║ ████╗      ║"));
    //sLog->outString(("║                       ╚═══╝  ╚══╝  ╚╝  ╚╝ ╚═══╝      ║"));
   sLog->outString(("AZcore With 3.3.5a"));
    //sLog->outString(("║  	                          FrozenCore With %s 3.3.5a       ║"), ssssss.c_str());
    //sLog->outString(("╚════════════════════════════════════╝"));
    sLog->outString("AuthServerVer 2022.12AZ ");

   // sLog->outString("   AuthServer版本：%s ", GitRevision::GetFileVersionStr());
