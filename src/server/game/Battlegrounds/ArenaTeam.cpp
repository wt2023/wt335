...
{
    // Returns how many points would be awarded with this team type with this rating  返回此级别的团队类型将获得多少积分（评级）
    float points;

    uint32 rating = memberRating + 150 < Stats.Rating ? memberRating : Stats.Rating;   //加？是判断号，意思就是如果是，选前面那个，如果否，选后面那个 （冒号前后）

    if (rating <= 1500)
    {
        if (sWorld->getIntConfig(CONFIG_ARENA_SEASON_ID) < 6)   //判断第几赛季，小于6则  （worldconf设置，里面目前是8）
    ...
    if (Type == ARENA_TEAM_2v2)
        ...
        points *= sWorld->getFloatConfig(CONFIG_ARENA_1V1_ARENAPOINTS_MULTI);   // （worldconf设置倍率）   344*倍率

    points *= sWorld->getRate(RATE_ARENA_POINTS);    // （worldconf设置最终获得的竞技点数倍率） 
...
{
    // called for each participant after winning a match  在赢得一场比赛后为每位参赛者调用
    for (MemberList::iterator itr = Members.begin(); itr != Members.end(); ++itr)
    {
        if (itr->Guid == player->GetGUID())
        {
...
{
    // Called after a match has ended and the stats are already modified 在比赛结束且统计数据已修改后调用
    // Helper function for arena point distribution (this way, when distributing, no actual calculation is required, just a few comparisons) 竞技场点数分布的帮助函数（这样，在分布时，不需要实际计算，只需要进行一些比较）
    // 10 played games per week is a minimum  每周至少玩10场游戏  我改成1了
    if (Stats.WeekGames < 1)  //被我从10改成1场
        return;

    // To get points, a player has to participate in at least 30% of the matches  为了获得积分，队员必须参加至少30%的比赛  我改成0
    //uint32 requiredGames = (uint32)ceil(Stats.WeekGames * 0.3f);
    uint32 requiredGames = 0;   //官方是30%的比赛，我改成0，即队员也可以获得比赛奖励


    for (MemberList::const_iterator itr = Members.begin(); itr != Members.end(); ++itr)
    {
        // The player participated in enough games, update his points  玩家参加了足够多的比赛，更新了他的积分
        uint32 pointsToAdd = 0;
        if (itr->WeekGames >= requiredGames)
            pointsToAdd = GetPoints(itr->PersonalRating);  //根据个人等级给他points

        std::map<uint32, uint32>::iterator plr_itr = playerPoints.find(GUID_LOPART(itr->Guid));
        if (plr_itr != playerPoints.end())
        {
            // Check if there is already more points  检查是否已经有更多的竞技点
...
{
    // Reset team stats  重置队伍比赛数据
    Stats.WeekGames = 0;
    Stats.WeekWins = 0;

    // Reset member stats  重置队员比赛数据
    for (MemberList::iterator itr = Members.begin(); itr != Members.end(); ++itr)
    {
        itr->WeekGames = 0;
        itr->WeekWins = 0;
