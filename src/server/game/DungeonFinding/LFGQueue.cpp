...
uint8 LFGQueue::FindGroups()  //找到队伍
{
...
LfgCompatibility LFGQueue::CheckCompatibility(Lfg5Guids const& checkWith, const uint64& newGuid, uint64& foundMask, uint32& foundCount, const std::set<Lfg5Guids>& currentCompatibles)
{
    ...
    for (uint8 i=0; i<5 && (guid=check.guid[i]) != 0 && numLfgGroups < 2 && numPlayers <= MAXGROUPSIZE; ++i)   //MAXGROUPSIZE
    {
        LfgQueueDataContainer::iterator itQueue = QueueDataStore.find(guid);
        if (itQueue == QueueDataStore.end())
        {
       ...
       numPlayers += itQueue->second.roles.size();
        //numPlayers = 5;
      //  int8 numLFGPlayers = MAXGROUPSIZE - numPlayers;
       uint8 numLFGPlayers = MAXGROUPSIZE - numPlayers;
       numPlayers = numLFGPlayers + itQueue->second.roles.size();
...
void LFGQueue::UpdateQueueTimers(uint32 diff)  //更新队列时间
{
