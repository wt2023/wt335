...
void ChallengeMod::Load()
{
		...
		"SELECT MapId, ChallengeLv, ReqId, GossipText FROM _challenge"))
	{
		do
		{
			Field* fields = result->Fetch();     //把当面行数据放入fields变量结构体里面
			ChallengeTemplate Temp;              //创建一个挑战表
			Temp.MapId = fields[0].GetUInt32();  //表的地图id=数据库记录fields
			Temp.Level = fields[1].GetUInt32();  //表的挑战等级=数据库记录fields
			Temp.ReqId = fields[2].GetInt32();   //表的需求id=数据库记录fields
			Temp.GossipText = fields[3].GetString(); //表的文本菜单=数据库记录fields
			ChallengeVec.push_back(Temp);           //表的数据传回挑战结构体
		} while (result->NextRow());//当下一行是真时，循环执行（即把全部数据匹配出来，并传入挑战结构体）
...
void ChallengeMod::ResetChallenge(uint32 guidlow, uint32 mapId)   //清除挑战的cd
{
	for (auto itr = PlayerChallengeVec.begin(); itr != PlayerChallengeVec.end();)
	{
		if (guidlow == itr->guid && mapId == itr->mapId)
			itr = PlayerChallengeVec.erase(itr);   //清除
		else
			++itr;
	}
}
...
int32 ChallengeMod::GetReqId(uint32 mapId, uint32 challengeLv) //获取需求模板ID
{
	for (auto itr = ChallengeVec.begin(); itr != ChallengeVec.end(); itr++)  //从挑战模板获取全部数据
		if (mapId == itr->MapId && challengeLv == itr->Level)   //如果地图id  = itrid    且挑战等级= itrlevel  反回itr的需求id
			return itr->ReqId;

	return 0;
}

std::string ChallengeMod::GetGossipText(uint32 mapId, uint32 challengeLv)  //获取对话框文本
{
...
void ChallengeMod::ChangeLevel(Player* leader, uint32 triggerId, uint32 challengeLv) //选择挑战等级
{
