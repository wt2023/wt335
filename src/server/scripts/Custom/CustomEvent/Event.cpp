...
public:
	...
	void OnCreatureKill(Player* player, Creature* creature)  //当击杀生物
	{
		if (!sEvent->Valid())
			return;
	...
	void OnUseGameObject(Player* player, GameObject* gob) //当使用物品
	{	
		if (!sEvent->Valid())
			return;
	...
	void OnPVPKill(Player* killer, Player* killed) //当pvp击杀
	{
		if (!sEvent->Valid())
			return;
	...
	void OnPlayerKilledByCreature(Creature* killer, Player* killed)   //当玩家被生物杀死
	{ 
		
	}

	void OnUpdateZone(Player* player, uint32 newZone, uint32 /*newArea*/)   //当更换区域zone
	{
		if (!player->InEvent())
			return;
	...
	void OnMapChanged(Player* player)  //当更换地图
	{
		if (!player->InEvent())
			return;
...
{
public:
	StageTrigger() : CreatureScript("StageTrigger") { }   //挑战npc
