...
#include <Conditions/DisableMgr.h>
#include "MapMod.h"
...
void MainFunc::Load()
{
	...
	do
	{
        ...
        else if (strcmp("随机挑战", str) == 0)
            Temp.FuncType = MF_RONDCHALL;
		else
...
void MainFunc::DoAction(Player* pl, Object* obj, uint32 sender, uint32 action)
{
	...
	case SENDER_ALT_CLASS:
		...
		CloseMenu(pl);
    case SENDER_RONDCHALL:
        sRondChall->Action(pl, action, obj);
		return;
	}
...
void RondChall::Load()
{
    ...
    QueryResult result = WorldDatabase.PQuery(
        "SELECT 菜单组ID,地图ID,挑战难度,传送坐标ID FROM __随机挑战地图");
    if (!result) return;
    do
    {
        Field* fields = result->Fetch();
        uint32 gosip = fields[0].GetUInt32();


        rond_template Temp;
        Temp.map_id = fields[1].GetUInt32();
        Temp.chall_level = fields[2].GetUInt32();
        Temp.pos_id = fields[3].GetUInt32();
        _chall_rond.insert(std::make_pair(gosip, Temp));
    } while (result->NextRow());

    _chall_goss.clear();
    QueryResult result1 = WorldDatabase.PQuery(
        "SELECT 菜单组ID,菜单描述 FROM __随机挑战菜单");
    if (!result1) return;
    do
    {
        Field* fields = result1->Fetch();
        uint32 ip = fields[0].GetUInt32();
        Goss_template Temp1;
        Temp1.goss_tt = fields[1].GetString();
        _chall_goss.insert(std::make_pair(ip, Temp1));
    } while (result1->NextRow());
    if (_chall_goss.empty())
    {
        sLog->outString("<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<随机挑战菜单数据为空<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<");

    }
    if (_chall_rond.empty())
    {
        sLog->outString("<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<,随机挑战数据为空<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<");
    }
    if (!_chall_rond.empty() && !_chall_goss.empty())
        sLog->outString("<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<,随机挑战数据读取完毕<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<");

}
string RondChall::GetText(uint32 gossid)
{
    auto tie = _chall_goss.find(gossid);
    if (tie != _chall_goss.end())
    {
        return tie->second.goss_tt;
    }

    return "";
}
void RondChall::RondChallLevel(Player* player, uint32 gossid)
{
    _challmap.clear();
    ...
    int i = 0;
    rond_type type;
    for (multimap<uint32, rond_template>::iterator it = _chall_rond.begin(); it != _chall_rond.end(); it++)
    {
        if (it->first == gossid)
        {
            type.chalevel = it->second.chall_level;
            type.mapid = it->second.map_id;
            type.posid = it->second.pos_id;
            _challmap.insert(std::make_pair(i, type));
            i++;
        }
    }
    ...
    if (!_challmap.empty())
    {
        ...
        uint32 pos = 0;
        time(0);
        map<uint32, rond_type>::iterator ite = _challmap.find(urand(0, _challmap.size() - 1));
        Map = ite->second.mapid;
        challengeLv = ite->second.chalevel;
        pos = ite->second.posid;
        int32 reqId = sChallengeMod->GetReqId(Map, challengeLv);
        AreaTriggerTeleport const* at = sObjectMgr->GetMapEntranceTrigger(Map);
        if (!at)
            return;

        for (auto itr = MapModVec.begin(); itr != MapModVec.end(); itr++)
        {
            if (Map == itr->Map && challengeLv == itr->ChallengeLv)
            {
                if (sReq->Check(player, itr->ReqId))
                {
                    sReq->Des(player, itr->ReqId);
                    if (MapEntry const* mapEntry = sMapStore.LookupEntry(Map))
                    {
                        sCF->ResetInstance(player, Difficulty(0), mapEntry->IsRaid(), Map);
                        sCF->ResetInstance(player, Difficulty(1), mapEntry->IsRaid(), Map);
                    }
                    sChallengeMod->ResetChallenge(player->GetGUIDLow(), Map);
                    sChallengeMod->ReplaceChallenge(player, Map, challengeLv);
                    if (pos > 0)
                    {
                        sMF->TelePort(player, pos);
                    }
                    else
                    {
...
void RondChall::RondResetChall(Player* player, uint32 gossid)
{
    for (multimap<uint32, rond_template>::iterator it = _chall_rond.begin(); it != _chall_rond.end(); ++it)
    {
        if (it->first == gossid)
        {
            sChallengeMod->ResetChallenge(player->GetGUIDLow(), it->second.map_id);
        }
    }
    return;
}
void RondChall::AddGossip(Player* player, Object* obj)
{
    player->PlayerTalkClass->ClearMenus();
    if (!_chall_goss.empty())
    {
        for (map<uint32, Goss_template>::iterator tie = _chall_goss.begin(); tie != _chall_goss.end(); tie++)
        {
            if (tie != _chall_goss.end())
            {
                player->ADD_GOSSIP_ITEM(0, GetText(tie->first), SENDER_RONDCHALL, GOSSIP_ACTION_INFO_DEF + tie->first);
            }
        }
...
void RondChall::Action(Player* player, uint32 action, Object* obj)
{
    if (action == GOSSIP_ACTION_INFO_DEF)
        AddGossip(player, obj);
    else
    {
        RondChallLevel(player, action - GOSSIP_ACTION_INFO_DEF);
    }
}
