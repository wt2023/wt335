...
//================================随机挑战FB模块=====================================================
struct Goss_template
{
    uint32 ID;
    string goss_tt;
};
struct rond_template
{
    uint32 goss_id;
    uint32 map_id;
    uint32 chall_level;
...
{
public:
    static RondChall* instance()
    {
        static RondChall instance;
        return &instance;
    }
    void Load();
    multimap<uint32, rond_template> _chall_rond;
    map<uint32, Goss_template>_chall_goss;
    map<uint32, rond_type> _challmap;
    string GetText(uint32 gossid);
    void RondChallLevel(Player* player, uint32 gossid);
    void RondResetChall(Player* player, uint32 gossid);
    void AddGossip(Player* player, Object* obj);
    void Action(Player* player, uint32 action, Object* obj);
private:

};
#define sRondChall RondChall::instance()
