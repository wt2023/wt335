...
bool MapMod::IsDisableSpell(uint32 spellId, Map* map)
{
    uint32 mapId = map->GetId();
    uint32 diff = map->GetDifficulty();
    uint32 challengelv = map->challengeLv;
    ...
    for (auto itr = MapModPlayerVec.begin(); itr != MapModPlayerVec.end(); itr++)
    {
        if (itr->Map == mapId && itr->Diff == diff && itr->ChallengeLv == challengelv)
        {
            if (std::find(itr->DisableSpells.begin(), itr->DisableSpells.end(), spellId) != itr->DisableSpells.end())
                return true;
            return false;
        }
    }
...
bool MapMod::IsDisableItem(uint32 spellId, Map* map)
{
    uint32 mapId = map->GetId();
    uint32 diff = map->GetDifficulty();
    uint32 challengelv = map->challengeLv;
    ...
    for (auto itr = MapModPlayerVec.begin(); itr != MapModPlayerVec.end(); itr++)
    {
        if (itr->Map == mapId && itr->Diff == diff && itr->ChallengeLv == challengelv)
        {
            if (std::find(itr->DisableItems.begin(), itr->DisableItems.end(), spellId) != itr->DisableItems.end())
                return true;
            return false;
        }
    }
...
void MapMod::RefreshAura(Player* player)
{
    ...
    for (uint8 i = EQUIPMENT_SLOT_START; i < EQUIPMENT_SLOT_END; i++)
    {
        if (Item* pItem = player->GetItemByPos(INVENTORY_SLOT_BAG_0, i))
        {
            ...
            for (uint8 i = 0; i < MAX_ITEM_PROTO_SPELLS; ++i)
            {
                ...
                bool hasAura = player->HasAura(proto->Spells[i].SpellId);
                bool disable = DisableMgr::IsDisabledFor(DISABLE_TYPE_SPELL, proto->Spells[i].SpellId, player);

                if (hasAura && disable)
                    player->RemoveAurasDueToSpell(proto->Spells[i].SpellId, pItem->GetGUID());
                    ...
                    for (int s = 0; s < MAX_ITEM_ENCHANTMENT_EFFECTS; ++s)
                    {
                        ...
                        if (enchant_display_type == ITEM_ENCHANTMENT_TYPE_EQUIP_SPELL)
                        {
                            ...
                            if (enchant_spell_id)
                            {
                                ...
                                if (hasAura && disable)
                                    player->RemoveAurasDueToSpell(enchant_spell_id, pItem->GetGUID());
                                else if (!hasAura && !disable)
                                {
                                    int32 basepoints = 0;
