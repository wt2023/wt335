...
struct MapModPlayerTemplate
{
    ...
    std::vector<uint32> DisableItems;
    std::vector<uint32> DisableSpells;
};
...
public:
    ...
    bool IsDisableSpell(uint32 spellId, Map* map);
    bool IsDisableItem(uint32 itemId, Map* map);

	void RemoveItem(Player* player);
    void RefreshAura(Player* player);
